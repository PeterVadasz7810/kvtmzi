...
def get_movie_details(filmID):
    ...
    for datas in Adatok:
        if datas['Data'] == 'Hossz':
            data_dictionary[datas['Data']] = datas['Content'].replace('\xa0','Nincs').replace("'"," perc").replace('"', ' másodperc')
        else:
...
def getmovies():
    ...
    for tags in tag.find_all("option"):
        ...
        moviedata = get_movie_details(tags.get("value"))  # eredetileg ez volt itt: dict(film=tags.string, id=tags.get("value"))
...
def filmlista_frissit():
    ...
    with conn:
        for film in movie_list:
            film_data = (film['Cím'],film['Alcím'],film['Rendező'],film['Alkotók'],film['Szereplők'],film['Év'],film['Hossz'],film['Leírás'],film['id'])
            if len(get_movieID(conn,[film['id']])) == 0:
                add_movie(conn,film_data)
            elif (len(get_field_null_value(conn,[film['id']])) != 0) or (len(title_changed(conn,[film['id'],film['Cím']])) != 0):
